...
export default function Hero({ heading, message, image, buttonText, buttonLink }) {
    return (
        <div className={`relative flex items-center justify-center h-[80vh] bg-fixed object-fill ${image}`}>
            {/* Overlay */}
            <div className="absolute inset-0 w-3/4 h-[80vh] bg-gradient-to-r from-black to-transparent z-[2]" />
            ...
            <div className="mt-10 animate-left-to-right">
                <Link href={buttonLink} className="hover:bg-white hover:text-black px-8 py-2 border">{buttonText}</Link>                </div>
            </div>
        </div>
    );
}